...
_lastStoryDiv:null,_lastNarrativeText:'',_paused:false,_writingStyle:'literary',
...
2. 第一幕必须是危机导入——主角在危险中醒来或被卷入
3. 每幕要有2-3个关键场景，每个场景有明确的挑战或抉择
4. 设计5-8个会在剧情中登场的NPC（含名字、身份、与主角的关系、动机）
5. 设计一条主线和2条暗线（玩家可能发现也可能错过）

返回JSON：
{"premise":"一句话核心悬念","mainPlot":"主线概要50字","subPlots":["暗线1","暗线2"],
"npcs":[{"name":"名","role":"身份","motivation":"动机","relation":"与主角关系","firstAppear":1}],
"chapters":[
{"title":"幕名","summary":"50字概要","scenes":[{"desc":"场景描述","challenge":"挑战","possibleOutcomes":["结果1","结果2"]}],"keyEvents":["事件"],"focusCharacters":["角色名"],"location":"地点","choicePoint":true}
]}`}],
...
const pc=AgentSystem.getAgent(this._playerCharacterId);
await this._narrateChapter(ch,`写一段300-500字的开场。"你"(${pc?.name})在危险中醒来。用感官细节（疼痛、声音、气味）。不要解释世界观。事件:${ch.keyEvents.join('、')}`)},
...
// ===== 玩家节奏控制 =====
async playerContinue(){UI.hideContinueButton();if(this._state!=='running'||this._paused)return;
const ch=this._plotOutline?.chapters?.[this._chapterIndex];if(!ch)return;
if(this._turnCount>0&&this._turnCount%3===0)await this._advanceChapter();
...
async makeChoice(text){this._choicesPending=false;UI.hideChoices();UI.clearTimer();
const pc=AgentSystem.getAgent(this._playerCharacterId);
